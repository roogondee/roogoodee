  ...
  return (
    <main className="min-h-screen bg-cream">
      <nav className="fixed top-0 left-0 right-0 z-50 px-10 py-5 flex justify-between items-center bg-cream/90 backdrop-blur-md border-b border-mint/15">
        <Link href="/" className="font-display text-2xl text-forest">รู้ก่อน<span className="text-mint italic">ดี</span></Link>
        <Link href="/contact" className="bg-forest text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:bg-sage transition-all">💬 ปรึกษาฟรี</Link>
      </nav>
      ...
      <article className="pt-32 pb-24 max-w-3xl mx-auto px-6">
        ...
        <h1 className="font-display text-4xl text-forest leading-tight mb-4">{post.title}</h1>
        <p className="text-muted text-sm mb-8">
          {post.published_at ? new Date(post.published_at).toLocaleDateString('th-TH', { year:'numeric', month:'long', day:'numeric' }) : ''}
        </p>
        <div
          className="prose prose-lg max-w-none text-rtext leading-relaxed [&>h2]:font-display [&>h2]:text-forest [&>h2]:text-2xl [&>h2]:mt-8 [&>h2]:mb-4 [&>p]:mb-4"
          dangerouslySetInnerHTML={{ __html: post.content || '' }}
        />

        {/* CTA */}
        <div className="mt-16 bg-gradient-to-br from-forest to-sage rounded-2xl p-8 text-white text-center">
          <h3 className="font-display text-2xl mb-2">ต้องการปรึกษาเพิ่มเติม?</h3>
          <p className="text-white/70 text-sm mb-6">ทีมผู้เชี่ยวชาญพร้อมตอบทุกคำถามค่ะ ฟรี ไม่ตัดสิน</p>
