...
def build_flex(post: dict, caption: str) -> dict:
    ...
    # Always link to the /lead/liff landing page directly. The page loads the
    # LIFF SDK from NEXT_PUBLIC_LIFF_ID and captures the user's LINE userId
    # when opened inside LINE, falling back to a plain form elsewhere. We avoid
    # the liff.line.me/<id>?key=val redirect form because the LINE client shows
    # a "system error" toast on that URL shape in several locale/version combos.
    lead_url = (
